...
tokens {
    ...
    DebuggerStatement   ;     Address               ;     Bool                    ;
    String              ; 
}
...
/*
contractDefinition
  : ( 'contract' | 'interface' | 'library' )  LT*   Identifier   LT*  
    ( 'is'   LT*  inheritanceSpecifier    LT*   (','  LT*  inheritanceSpecifier )* )?
    LT*   '{'    LT*  (contractPart)*   LT*  '}'   ;
*/
/*
contractDefinition
	:  'contract' LT*    Identifier   LT*   '{'    LT*  (contractPart)*   LT*  '}'   ;
...
inheritanceSpecifier
  : userDefinedTypeName   LT* ( '('   LT*  expression   LT*  ( ','  LT*  expression )*  LT*  ')' )? ;
...
// functions
functionDeclaration:
	 'function' LT* Identifier  LT* formalParameterList LT*  '{' LT* functionBody? '}' -> ^(Function  formalParameterList ^(Statements functionBody?) ^(Ident Identifier))
	;
	
functionExpression:
	 'function' LT* Identifier? LT* formalParameterList LT*  '{'  LT* functionBody? '}' -> ^(FunctionExpr  formalParameterList ^(Statements functionBody?) ^(Ident Identifier)?)
	;


returnParameters
  : 'returns' '(' (LT* elementaryTypeName (LT* ',' LT* elementaryTypeName)*)+ LT* ')' ;
...
formalParameterList:
	         '('      LT*            ')'                               ->  ^(Listarguments)
      |  '(' (LT* Identifier (LT* ',' LT* Identifier)*)+ LT* ')'  ->  ^(Listarguments ^(Ident Identifier)+)
	;
...
variableStatement:
        'var'           storageLocation? LT* variableDeclarationList (LT | ';')* -> ^(Var storageLocation? variableDeclarationList)
//	| 'let' storageLocation?  LT* variableDeclarationList (LT | ';')* -> ^(Let storageLocation? variableDeclarationList)
       | 'address'  storageLocation?  LT* variableDeclarationList (LT | ';')* -> ^(Address storageLocation? variableDeclarationList)
	| Int             storageLocation?  LT* variableDeclarationList (LT | ';')* -> ^(Int storageLocation? variableDeclarationList)
	| Uint          storageLocation?  LT* variableDeclarationList (LT | ';')* -> ^(Uint storageLocation? variableDeclarationList)
	| Byte         storageLocation?  LT* variableDeclarationList (LT | ';')* -> ^(Byte storageLocation? variableDeclarationList)
	| Fixed        storageLocation?  LT* variableDeclarationList (LT | ';')* -> ^(Fixed storageLocation? variableDeclarationList)
	| Ufixed      storageLocation?  LT* variableDeclarationList (LT | ';')* -> ^(Ufixed storageLocation? variableDeclarationList)
	|'bool'        storageLocation?  LT* variableDeclarationList (LT | ';')* -> ^(Bool storageLocation? variableDeclarationList)
	|'string'     storageLocation?  LT* variableDeclarationList (LT | ';')* -> ^(String storageLocation? variableDeclarationList)
	| 'byte'      storageLocation?  LT* variableDeclarationList (LT | ';')* -> ^(Byte storageLocation? variableDeclarationList)
	;
...
variableDeclarationListNoIn:
    v1=variableDeclarationNoIn (LT* ',' LT* variableDeclarationNoIn)* -> variableDeclarationNoIn+
	;
...
letStatement :
	'let'  LT* '('  variableDeclarationList  ')'  LT* statement? (LT | ';')? -> ^(Let variableDeclarationList statement?) 
	;

ifStatement :
  'if' LT* '(' LT* expression LT* ')' LT*  statement  (LT*  'else' LT* 'if'  '(' LT* expression LT* ')'  LT* statement)*   (LT*  'else'  LT*  statement)? -> ^(If expression statement ?  statement*  statement?)
	;
...
doWhileStatement:
   'do' LT* statement LT* 'while' LT* '(' expression ')' (LT | ';')? -> ^(DoWhile statement expression)
	;
...
forStatement:
   'for' LT* '(' ( LT* forStatementInitialiserPart)? LT*  ';' ( LT* e1=expression)? LT* ';' ( LT* e2=expression)? LT* ')' LT* statement -> ^(For  ^(Expr  forStatementInitialiserPart?)   ^(Expr $e1?) ^(Expr $e2?) statement )
	;
...
forInStatement:
 'for'  LT* Identifier? LT* '(' LT* forInStatementInitialiserPart LT* 'in' LT* expression LT* ')' LT* statement -> ^(ForIn  ^(Expr forInStatementInitialiserPart?)   ^(Expr expression)  ^(Each Identifier?) statement)
	;
...
caseBlock:
   '{'! (LT!* caseClause)* LT!* (LT!* defaultClause (LT!* caseClause)*)? LT!* '}'! //-> caseClause*  defaultClause ?
	;
...
tryStatement:
   'try' LT* statementBlock LT* (finallyClause |  (catchClause LT* catchClause* (LT* finallyClause)?) ) -> ^(Try statementBlock catchClause* finallyClause?)
	;

catchClause:
   'catch' LT* '(' LT* Identifier LT* ('if' expression) ?  LT* ')' LT* statementBlock -> ^(CatchClause ^(Ident Identifier) statementBlock expression?)
	;
...
arrayLiteral
	: '[' LT* assignmentExpression? (LT* ',' (LT* assignmentExpression)?)* LT* ']' -> ^(List assignmentExpression*)
	;
arrayLiteralCreation
	: '[' LT* assignmentExpression LT* forInList LT* ']' -> ^(ListCreation assignmentExpression forInList)
	;
forInList:
 'for' LT* Identifier? LT* '(' LT* forInStatementInitialiserPart LT* 'in' LT* expression LT* ')' -> ^(ForIn  ^(Expr forInStatementInitialiserPart?) ^(Expr expression) ^(Each Identifier?) )
	;

// objectLiteral definition.
objectLiteral
	: '{' LT* propertyNameAndValue? (LT* ',' LT* propertyNameAndValue)* LT* '}'-> ^(ObjectLiteral propertyNameAndValue*)
	
	;
propertyNameAndValue:
	  Identifier LT* propertyName LT* '(' LT* ')' LT* '{' LT* functionBody '}'  -> ^(PropertyGet Identifier propertyName functionBody)
	| a=Identifier LT* propertyName LT* '(' LT* b=Identifier  LT* ')' LT* '{' LT* functionBody '}'-> ^(PropertySet $a propertyName ^(Ident $b) functionBody)
	| propertyName LT* ':' LT* assignmentExpression -> ^(PropertyName propertyName assignmentExpression)
