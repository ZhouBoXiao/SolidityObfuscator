...
#RESERVED_WORDS = analyze.reserved + analyze.keywords + analyze.modifier_words
...
def find_obfuscatables(tokens, obfunc):
    ...
    *obfunc* is expected to return the token string if that token can be safely
    ...
    # global imported_modules
    # = analyze.enumerate_imports(tokens)
...
def obfuscatable_class_function(tokens, index, **kwargs):
    """
    Given a list of *tokens* and an *index* (representing the current position),
    ...
    lis = ['contract', 'function', 'event', 'modifier', 'library']
    if index > 0:
        prev_tok = tokens[index-1]
    else: # Pretend it's a newline (for simplicity)
        prev_tok = (54, '\n', (1, 1), (1, 2), '#\n')
...
def obfuscate_variable(tokens, index, replace, replacement, right_of_equal, inside_parens, inside_function):
    ...
    *replacement*. *right_of_equal*, and *inside_parens* are used to determine
    ...
    if prev_tok_string == '.' and pre_prev_tok_string in ('msg', 'abi', 'block', 'tx'):
        return None
    if prev_tok_string == '.' and token_string in ('balance', 'send', 'transfer', 'length', 'push'):
        return None
    if token_string == "import" or token_string == 'pragma':
        return '__skipline__'
    if next_tok[1] == '.':
        if token_string in analyze.global_variable:
            return None
...
def obfuscate(module, tokens, options, name_generator=None, table=None):
    ...
    if options['obfuscate']:
        ...
        classes = find_obfuscatables(tokens, obfuscatable_class_function)
        for variable in variables:
            replace_obfuscatables(module, tokens, obfuscate_variable, variable, name_generator, table)

        for _class in classes:
            replace_obfuscatables(module, tokens, obfuscate_class, _class, name_generator, table)
