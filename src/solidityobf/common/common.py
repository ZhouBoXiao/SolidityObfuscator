from solidityobf.common.classes import *
from solidityobf.common.finder import getfirstident
import tree_walker
import cPickle
...
def ident_is_in_this_property(program):
    return (check_depth_type(program, 1, Property) and
        ...
        isinstance( program.parent.parent.expr,  Expr))
#and isinstance( program.parent.parent.expr.exprs[0], This))
...
def is_break_inside_weak(program):
    return is_element_inside(program, Break, [Function])
...
def detect_presence(functionorlist):
    ...
    args = {"this_present": False, "break_present": False, "eval_present": False, "function_init": functionorlist,
            "arguments": False}
...
def examine_function_vars_func(parse_tree, args):
    ...
    if parse_tree == args["till"]:
        args["continue"] = False
        return [], args
    if parse_tree.__class__ in [VarDeclaration, Function]:
        ident = None
        assigned = False
        ...
        if isinstance(parse_tree, VarDeclaration):
            ...
            assigned = parse_tree.expr is not None
...
def examine_function_vars(functionorlist, tillencounter=None):
    walk_condition = lambda x, y: y["continue"] and (not isinstance(x, Function) or y["function_init"] == x)
    args = {"function_init": functionorlist, "var_dec": {}, "var_dec_and_assigned": {}, "ref": {}, "thisdotref": {},
            "assignment": {}, "decassignment": {}, 'continue': True, "till": tillencounter}
    if type(functionorlist) != list:
        functionorlist = [functionorlist]
    for s in functionorlist:
        ret, args = tree_walker.walker(s, examine_function_vars_func, examine_function_vars_func_post, arg=args,
    ...
    if isinstance(functionorlist, Program):
        for k in var_global_set.keys():
            ...
            if k in local_var:
                ...
                local_var[k] = var_global_set[k]
        var_global_set = {}
    return {"local_var": local_var, "var_global_ref": var_global_ref, "var_global_set": var_global_set,
