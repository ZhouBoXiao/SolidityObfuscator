...
from solidityobf.common.colors import blue
...
class GenericRunner(object):
    ...
    def create_reference_executable(self):
        self.reference_executable = getnewfilename(directory=self.temp_dir, prefix=self.temp_progam_name, postfix=".js")
        f = open(self.reference_executable, "w")
        f.write(open(self.program_path).read()+self.extra_end)
        f.close()
        
    def create_file_from_tree(self):
        self.modified_executable = getnewfilename(directory=self.temp_dir, prefix=self.temp_progam_name, postfix=".js")
    ...
    def compute_default_result(self):
        if self.answer is None:
            ...
            answer, cmd_res2 = execute_command(self.interpreter + " " + self.reference_executable, get_res=True)
            if cmd_res2 != 0:
                if self.verbose >= 2:
                    print (self.program_path, self.interpreter, "return a non-zero value")
            self.answer = answer
    ...
    def runner(self, program_tree):
        ...
        result, tim, cmd_res = execute_command(self.interpreter +
            " " + self.modified_executable, verbose=self.verbose, get_time=True, get_res=True)
