...
functionDefinition
  : 'function' LT* identifier LT* parameterList LT*  '{' LT* statement* '}' ;
...
typeName
  : (elementaryTypeName | userDefinedTypeName | mapping | functionTypeName | 'address' 'payable') ('[' expression? ']')* ;
...
expression
  : ('new' typeName | '(' expression ')' | ('++' | '--') expression | ('+' | '-') expression | ('after' | 'delete') expression | '!' expression | '~' expression | primaryExpression) (('++' | '--') | '[' expression ']' | '(' functionCallArguments ')' | '.' identifier | '**' expression | ('*' | '/' | '%') expression | ('+' | '-') expression | ('<<' | '>>') expression | '&' expression | '^' expression | '|' expression | ('<' | '>' | '<=' | '>=') expression | ('==' | '!=') expression | '&&' expression | '||' expression | '?' expression ':' expression | ('=' | '|=' | '^=' | '&=' | '<<=' | '>>=' | '+=' | '-=' | '*=' | '/=' | '%=') expression)* ;
