from solidityobf.common.classes import (Statements, VarDeclaration, This, Break, Ident, Assignment,
                                        Function, Functioncall, Expr, ExpressionStatement, Listarguments, Index, Number, Var,
                                        MemberExpr, Return, List)
from solidityobf.common.common import is_return_inside,is_function_inside,detect_presence,examine_function_vars,include_ident, solidityobf_random
from solidityobf.common.finder import get_all_ident,get_upper_function,get_all_upper_statement_and_pos
from solidityobf.common.common import deepcopy,removeindex,ident_is_property,ident_is_in_left_handside_assignment
from solidityobf.common.colors import BLACK,RED
...
def outlining_post_func(program,arg):
    if isinstance(program,Statements):
        if shouldi(arg=arg) and len(program.statements_list)!= 0:
            f = solidityobf_random.randint(0, len(program.statements_list) - 1)
            t = solidityobf_random.randint(f + 1, len(program.statements_list))
            ...
            if not is_return_inside(outline) and not is_function_inside(outline):
                ...
                if not (presence["arguments"] or presence["this_present"] or presence["break_present"] or presence["eval_present"] ):
                    ...
                    upper , pos = uppers[solidityobf_random.randint(0, len(uppers) - 1)]
