from solidityobf.common.classes import Number,Functioncall,Expr,Ident, List
...
def modify_data_flow_2_func(program, arg):
    if isinstance(program, Number):
        if shouldi(arg=arg):
            ...
            if s != None:
                whereto = solidityobf_random.randint(0, pos)
    ...
    if isinstance(program, List):
        if shouldi(arg=arg):
            s, pos = get_upper_statement_and_pos(program)
            if s is not None and not is_functioncall_inside(program):
                whereto = solidityobf_random.randint(0, pos)
