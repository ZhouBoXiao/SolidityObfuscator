from solidityobf.common.classes import String,PropertyName, Var, Ident, Expr, VarDeclaration, Functioncall, Listarguments
    ...
    if isinstance(program, String):
        if len(program.value) > 1:    
            ...
            if list_upper_statements != [] and (not isinstance(program.parent, PropertyName)): # and program != program.parent.name):
                if shouldi(arg=arg):
                    depth = solidityobf_random.randint(0, len(list_upper_statements) - 1)
